...
sq_l = 20 if retrain else 1
b_s = 64 if retrain else 1
model_test = bsc_run.define_model(seq_len=sq_l, batch_size=b_s,
                                  obs_size=obs_size, act_size=act_size,
                                  stateful=sl_kwargs['stateful'],
                                  num_h=n_lstm,
                                  loss=sl_kwargs['loss'])
files = glob.glob(folder+'*model*steps')
assert len(files) == 1
# ld_f = folder+'model_'+str(stps_ep)+'_steps'.replace('//', '/')
print('loading: ', files[0])
model_test.load_weights(files[0])
...
if retrain:
    ...
    model_test.save(files[0]+'plus_'+str(stps_ep))
