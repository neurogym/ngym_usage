...
def get_dataset_for_SL(env_name, kwargs, rollout, n_tr=1000000,
    ...
    if verbose:
        ...
        print("Task: ", env_name)
        print(
            "Producing dataset with {0} steps".format(TOT_TIMESTEPS)
    ...
    if count_stps % rollout:
        discard_incomplete_obs = samples.size % (
            rollout * OBS_SIZE
        )  # so this can lead to empty sample/target!!?
        discard_incomplete_act = target.size % (rollout * ACT_SIZE)
        samples = samples.flatten()[:-discard_incomplete_obs].reshape(
            (-1, rollout, OBS_SIZE)
        )  # crashes because random size
        target = target.flatten()[:-discard_incomplete_act].reshape(
            (-1, rollout, ACT_SIZE)
        )
    else:  # original
...
def train_env_keras_net(
    ...
    for ind_ep in range(num_ep):
        ...
        try:
            ...
            print(
                f"iter {ind_ep} in train\nsamples shape= {samples.shape}\ntarget_shape = {target.shape}"
            )
            raise ValueError(e)
        # test
        samples = np.empty(0)  # temporary workaround
        while samples.size == 0:
            samples, target, env = get_dataset_for_SL(
                env_name=env_name,
                kwargs=kwargs,
                rollout=rollout,
                n_tr=tr_per_ep,
    ...
    data = {"acc": acc_training, "loss": loss_training, "perf": perf_training}
...
def eval_net_in_task(
    ...
    env.seed(seed=seed)
    if save:
        env = monitor.Monitor(env, folder=folder, num_tr_save=ntr_save)
    obs = env.reset()
    perf = []
    actions_plt = []
    rew_temp = []
    ...
    for ind_act in range(num_steps - 1):
        ...
        if (
            len(action.shape) > 1
        ):  # packaged actions will lead to +1 dim and crash upon eval
